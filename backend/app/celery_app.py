...
# Beat 定时任务配置（窗口热点预热：不落库）
if settings.WINDOW_HOTSPOT_BEAT_ENABLED:
    parts = [p.strip() for p in (settings.WINDOW_HOTSPOT_CRON or "").split() if p.strip()]
    if len(parts) != 5:
        raise ValueError("WINDOW_HOTSPOT_CRON 格式错误，需为 5 段 cron：minute hour dom month dow")

    minute, hour, day_of_month, month_of_year, day_of_week = parts

    celery_app.conf.beat_schedule = {
        "build-window-hotspots": {
            "task": "app.tasks.window_hotspots.build_window_hotspots_task",
            "schedule": crontab(
                minute=minute,
                hour=hour,
